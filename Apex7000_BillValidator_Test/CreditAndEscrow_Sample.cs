...
partial class MainWindow
{
    ...
    {
        _apexValidator.Stack();


        if (UsdBillValues.ContainsKey(lastIndex))
            Console.WriteLine("Escrowed ${0}", UsdBillValues[lastIndex]);
        else
    ...
    private void btnReject_Click(object sender, RoutedEventArgs e)
    {
        ...
        _apexValidator.Reject();


        if (UsdBillValues.ContainsKey(lastIndex))
            Console.WriteLine("Rejected ${0}", UsdBillValues[lastIndex]);
        else
    ...
    private void validator_OnCredit(object sender, CreditArgs e)
    {
        ...
        if (UsdBillValues.ContainsKey(denomination))
        {
            if (denomination > 0)
                Console.WriteLine("Credited ${0}", AddBillValue(denomination));
            else
